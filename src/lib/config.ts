const normalizeApiBase = (url: string | undefined, fallback: string) => {
  const s = url?.trim() || fallback;
  return s.replace(/\/$/, "");
};

export const config = {
  API_URL: normalizeApiBase(process.env.NEXT_PUBLIC_API_URL, "http://localhost:4001"),
...
export function setAuthData(token: string, user: unknown, tenant: unknown) {
  ...
  document.cookie = `authToken=${token}; path=/; max-age=${3 * 24 * 60 * 60}; SameSite=Lax`;
}
